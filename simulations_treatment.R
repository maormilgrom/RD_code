...
results.so<- as.data.frame(matrix(0, ncol = 6, nrow = loop))
colnames(results) <- c("coef","obw","obw_l","obw_r",
                       "coef_diff","obw_diff")
...
for(i in 1:loop) {
  ...
  results.current=rdrobust(sample$y.so,sample$x)
  results.so[i,1]=results.current$coef[bc+1]-jump # normalizing to zero
  results.so[i,3:4]=results.current$bws[bc+1,1:2]
  results.so[i,2]=results[i,3]+results[i,4]
  results.so[i,5:6]=results[i,1:2]-results.so[i,1:2]
  }
  
  if (treat=="zero") {
    sample.treat=sample
    sample.treat$y.treat <- ifelse(sample$x> -results[i,7] & 
  ...
  results.current=rdrobust(sample.treat$y.treat,sample.treat$x)
  ...
  if (i <= 5) {
    ...
    figure_name=paste("figures/sample_so",i,"_",treat,save.ext,".png",sep = "")
    png(figure_name)
    sample.treat %>%
      filter(x > -20 & x < 20) %T>%
      plot(y.so~x,., ylim = range(c(y.so,y.treat)),
           col="red", ylab = "Y", xlab = "X") %T>%
      par(new = T) %>%
      plot(y.treat~x,., ylim = range(c(y.so,y.treat)),
    ...
    figure_name=paste("figures/sample_so",i,"_",treat,"_",save.ext,".png",sep = "")
    figure_name=paste("figures/sample_so",i,treat,save.ext,"model.png",sep = "_")
    png(figure_name)
    temp.treat=sample.treat %>%
      filter(x > -20 & x < 20)
    temp=sample %>%
      filter(x > -20 & x < 20)
    plot(temp$y.so~temp$x, ylim = range(c(temp.treat$y.so,temp$y.so)),
         xlim = range(c(temp.treat$x,temp$x)),
         col="red", ylab = "Y", xlab = "X")
    par(new = T)
    plot(temp.treat$y.so~temp.treat$x, ylim = range(c(temp.treat$y.so,temp$y.so)),
         xlim = range(c(temp.treat$x,temp$x)),
         col="blue", ylab = "Y", xlab = "X")
    par(new = T)
    plot(temp$y.model~temp$x, ylim = range(c(temp.treat$y.so,temp$y.so)),
         xlim = range(c(temp.treat$x,temp$x)),
    ...
    figure_name=paste("figures/rd_plot_so",i,"_",save.ext,".png",sep = "")
    png(figure_name)
